	...
	ordererService := Service{
		...
		Volumes: []string{
			...
			fmt.Sprintf("%s/crypto-config/ordererOrganizations/%s/orderers/%s/tls/:/var/hyperledger/orderer/tls",
				filePath, orderer.Domain, orderer.GetHostname()),
			fmt.Sprintf("%s:/var/hyperledger/production/orderer", filepath.Join(orderer.Dest, "production", orderer.GetHostname())),
		},
...
func GeneratePeerDockerComposeFile(filePath string, peer *network.Node, gossipBootstrapPeerUrl string, otherUrls []string, imageTags []string, couchdb bool) error {
	...
	peerService := Service{
		...
		Volumes: []string{
			...
			fmt.Sprintf("%s:/var/hyperledger/production", filepath.Join(peer.Dest, "production", peer.GetHostname())),
		},
